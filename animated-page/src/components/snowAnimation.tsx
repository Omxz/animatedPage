...
interface LayerProps {
  ...
  opacity: number;
  blur: number;
  colorVariationMin: number;
  colorVariationMax: number;
  symbols: string[];
  zIndex: number;
}
...
class SnowLayer {
  ...
  snowPileHeights: number[];
  SEGMENT_WIDTH: number;
  NUM_SEGMENTS: number;

  constructor(canvas: HTMLCanvasElement, layerProps: LayerProps) {
    this.canvas = canvas;
  ...
  createSnowflakes(numFlakes: number) {
  ...
  addToSnowPile(x: number, size: number) {
    const index = Math.floor(x / this.SEGMENT_WIDTH);
...
const SnowAnimation: React.FC = () => {
  ...
  useEffect(() => {
    ...
    LAYERS.forEach((layer, index) => {
      ...
      if (canvas) {
        const snowLayer = new SnowLayer(canvas, layer);
  ...
  return (
    <>
      ...
      {LAYERS.map((layer) => (
        <canvas
          key={`snow-canvas-${layer.layer}`}
          id={`snow-canvas-${layer.layer}`}
          ref={(el) => {
            if (canvasRefs.current) {
              canvasRefs.current[layer.layer - 1] = el;
            }
          }}
